...
### Requirement: availableXxxOrFail methods on service classes

**Reason**: Replaced by `canOrFail` from `BaseUserAccess`
**Migration**: Use `canOrFail(userId, Action, context?)` with the appropriate `Action` enum value and `ActionContextDto` context